...
export function getPreviousSun(date) {
    let dateIndex = getCorrectDate(date);
    let safetyIndex = 0;
    while (dateIndex.toLocaleString('default', {weekday: 'short'}) !== "Sun") {
        dateIndex.setDate(dateIndex.getDate() - 1);
        if (safetyIndex++ > 10) {
            console.error('getPreviousSun() stuck in a loop. date = ' + date);
            break;
        }
    }
    return dateIndex;
...
export function getNextSat(date) {
    let dateIndex = getCorrectDate(date);
    let safetyIndex = 0;
    while (dateIndex.toLocaleString('default', {weekday: 'short'}) !== "Sat") {
        dateIndex.setDate(dateIndex.getDate() + 1);
        if (safetyIndex++ > 10) {
            console.error('getNextSat() stuck in a loop. date = ' + date);
            break;
        }
    }
    return dateIndex;
